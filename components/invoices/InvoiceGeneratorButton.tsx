...
export function InvoiceGeneratorButton({ proposals }: Props) {
  ...
  async function handleSend() {
    ...
    if (!result.success) { setStatus("error"); setErrorMsg(result.error); return; }
    if (!result.emailSent) { setStatus("emailFailed"); setErrorMsg(result.emailError); return; }
    setStatus("success");
  }
          ...
          <div className="glass rounded-2xl w-full max-w-[540px] flex flex-col max-h-[90vh] shadow-[0_24px_48px_rgba(0,0,0,0.15)]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/50 flex-shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[15px] font-bold text-[#0f172a]">New Invoice</span>
              </div>
            ...
            <div className="p-6 flex flex-col gap-4 overflow-y-auto">
              {status === "success" && (
                <div className="flex items-center gap-2 text-[13px] text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                  <CheckCircle className="w-4 h-4 flex-shrink-0" /> Invoice sent to {email}
                </div>
              )}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[12.5px] font-semibold text-[#0f172a]">Linked Proposal</label>
                <select
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addItem} className="flex items-center gap-1 text-[11.5px] font-semibold text-[#1B3FEE] hover:opacity-75 transition-opacity">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-[1fr_120px_28px] gap-2 items-center">
                      <input type="text" value={item.description} onChange={(e) => updateItem(idx, "description", e.target.value)} placeholder="Service description" className={inputCls} />
                      <input type="number" min="0" step="0.01" value={item.amount || ""} onChange={(e) => updateItem(idx, "amount", e.target.value)} placeholder="0.00" className={inputCls} />
                      <button type="button" onClick={() => removeItem(idx)} disabled={items.length === 1} className="w-7 h-7 flex items-center justify-center rounded-lg text-[#94a3b8] hover:text-[#ef4444] hover:bg-[rgba(239,68,68,0.08)] transition-colors disabled:opacity-30">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[12.5px] font-semibold text-[#0f172a]">Notes <span className="font-normal text-[#94a3b8]">(optional)</span></label>
                <textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Payment instructions, thank you message…" className={`${inputCls} resize-none`} />
              </div>
            </div>
            ...
            <div className="flex gap-2 px-6 py-4 border-t border-white/50 flex-shrink-0">
              ...
              <Button type="button" variant="primary" className="flex-1" loading={loading} onClick={handleSend} disabled={status === "success"}>
