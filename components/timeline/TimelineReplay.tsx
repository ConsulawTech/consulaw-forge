...
export function TimelineReplay({ clients }: { clients: ClientOption[] }) {
  ...
  useEffect(() => {
    if (timelineData) setTasks(timelineData.tasks);
  }, [timelineData]);
    ...
    <div>
      ...
      <div className="glass rounded-2xl p-4 mb-5">
        <div className="flex items-end gap-4 flex-wrap">
          ...
          {selectedClientId && (
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                ...
                placeholder={clientProjects.length === 0 ? "No projects yet" : "Select a project…"}
          ...
          {timelineData && !loading && (
            <div className="ml-auto flex flex-col items-end pb-0.5">
              <span className="text-[13px] font-bold text-[#0f172a]">
                {timelineData.clientName} · {timelineData.projectName}
              </span>
              <span className="text-[11px] text-[#94a3b8]">
                {totalDays} day project · {timelineData.tasks.length} task{timelineData.tasks.length !== 1 ? "s" : ""}
              </span>
            </div>
          )}
        </div>
      ...
      {!timelineData && !loading && (
        <div className="glass rounded-2xl flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-[16px] font-bold text-[#0f172a] mb-1.5">No project selected</h3>
          <p className="text-[13px] text-[#94a3b8] max-w-[320px] leading-relaxed">
            {!selectedClientId
              ? "Choose a client from the selector above, then pick a project to visualise its task timeline."
              : !selectedProjectId
              ? "Now choose a project to load its tasks into the timeline."
              : "Loading…"}
          </p>
        </div>
      )}
      ...
      {timelineData && !loading && (
        <>
          ...
          <div className="flex items-center justify-between mb-2.5 flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[12px] font-semibold text-[#475569] uppercase tracking-[0.06em]">
                Task board — drag to update status
              </span>
              {playing && (
          ...
          <div className={`grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-5 transition-opacity duration-200 ${isPending ? "opacity-70" : ""}`}>
                          ...
                          return (
                            <span
                              ...
                              style={{
                                background: bg,
                                color,
                                borderColor: s === "done" ? "rgba(16,185,129,0.2)" : s === "active" ? "rgba(27,63,238,0.15)" : "rgba(203,213,225,0.4)",
                              }}
                            >
                              {t.label}
                            </span>
