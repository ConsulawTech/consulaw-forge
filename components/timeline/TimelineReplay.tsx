...
export function TimelineReplay({ clients }: { clients: ClientOption[] }) {
  ...
  // Slider date labels — only meaningful when a project is loaded
      ...
      {!timelineData && !loading && (
        <div className="glass rounded-2xl flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-[13px] text-[#94a3b8] max-w-[320px] leading-relaxed">
            {!selectedClientId
              ? "Choose a client from the selector above, then pick a project to visualise its milestone timeline."
              : !selectedProjectId
              ? "Now choose a project to load its milestones and tasks into the timeline."
              : "Loading…"}
          </p>
        </div>
      )}

      {/* ── Content — only shown when a project is loaded ── */}
      {timelineData && !loading && (
        <>
          {/* Stats */}
          ...
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-[12px] font-semibold text-[#475569] uppercase tracking-[0.06em]">
              Task board — drag between phases
            </span>
